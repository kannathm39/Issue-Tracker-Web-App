            ...
            <div class="center-container">
                ...
                if (isset($_POST['username']) && isset($_POST['fname']) && isset($_POST['lname']) && isset($_POST['email']) && isset($_POST['password']) && isset($_POST['passwordconf'])) {
                    ...
                    if (strlen($_POST['username']) >= 3 && strlen($_POST['username']) <= 32) {
                        //Firstname length
                        if (strlen($_POST['fname']) >= 1 && strlen($_POST['fname']) <= 32) {
                            //Lastname length
                            if (strlen($_POST['lname']) >= 1 && strlen($_POST['lname']) <= 32) {
                                //Password length
                                if (strlen($_POST['password']) >= 8 && strlen($_POST['password']) <= 25) {
                                    ...
                                    if ($_POST['password'] == $_POST['passwordconf']) {
                                        ...
                                        if (preg_match('/(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}/', $_POST['password'])) {
                                            if (strlen($_POST['email']) >= 3 && strlen($_POST['email']) <= 50) {
                ...
                <form action="./index.php" method="post" target="_self" class="form-log user-form">
                        ...
                        <input type="text" id="username" name="username" minlength="3" maxlength="32" required value="<?php echo isset($_POST['username']) ? htmlspecialchars($_POST['username'], ENT_QUOTES) : ''; ?>"><br><br>
                        <label for="fname">First name:</label>
                        <input type="text" id="fname" name="fname" minlength="1" maxlength="32" required value="<?php echo isset($_POST['fname']) ? htmlspecialchars($_POST['fname'], ENT_QUOTES) : ''; ?>"><br><br>
                        <label for="lname">Last name:</label>
                        <input type="text" id="lname" name="lname" minlength="1" maxlength="32" required value="<?php echo isset($_POST['lname']) ? htmlspecialchars($_POST['lname'], ENT_QUOTES) : ''; ?>"><br><br>
                        <label for="email">Email address:</label>
                        <input type="email" id="email" name="email" minlength="3" maxlength="50" required value="<?php echo isset($_POST['email']) ? htmlspecialchars($_POST['email'], ENT_QUOTES) : ''; ?>"><br><br>
